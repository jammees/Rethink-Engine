...
function RigidBody.new(
	...
	self.UpdateFinished = Signal.new()
	...
	if not self.custom then
		...
		-- self._janitor:Add(UiPool.ObjectReturned:Connect(function(object: Instance)
...
function RigidBody:Clone(deepCopy: boolean)
	...
	-- Copy lifespan, states and filtered RigidBodies, and other properties!!!!
	if deepCopy == true then
		...
		copy:KeepInCanvas(self.KeepInCanvas)
		copy:SetGravity(self.Gravity)
		copy:SetFriction(self.Friction)
		copy:SetAirFriction(self.AirFriction)

		if not self.custom then
			copy:CanRotate(self.CanRotate)
		end
	end
