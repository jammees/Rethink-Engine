...
Animator.Tools = {
	...
	["PopulateRow"] = function(iterations: number): { [number]: number }
		Log.TAssert(t.number(iterations))
...
function Animator:AddSpritesheet(
	...
	Log.TAssert(t.union(t.string, t.number)(imageId))
	Log.TAssert(t.Vector2(size))
	Log.TAssert(t.table(animations))
...
function Animator:AddCollection(collection: { [string]: number })
	Log.TAssert(t.table(collection))
...
function Animator:Play(): Promise
	if not self.CurrentAnimation then
		return Log.Warn(DebugStrings.Animator.NoAnimation)
	end

	if not self.Objects[1] then
		return Log.Warn(DebugStrings.Animator.NoObjectsAttached)
	end
...
function Animator:SetFramerate(framerate: number)
	Log.TAssert(t.number(framerate))
...
function Animator:NextFrame()
	...
	if self.Running then
		return Log.Warn((DebugStrings.Animator.AnimationRunning):format(":NextFrame()"))
	end
...
function Animator:PreviousFrame()
	...
	if self.Running then
		return Log.Warn((DebugStrings.Animator.AnimationRunning):format(":PreviousFrame()"))
	end
...
function Animator:AttachObject(object: ImageLabel | ImageButton)
	Log.TAssert(t.union(t.instanceOf("ImageLabel"), t.instanceOf("ImageButton"))(object))

	if table.find(self.Objects, object) then
		return Log.Warn(DebugStrings.Animator.ObjectAlreadyAttached)
	end
...
function Animator:DetachObject(object: ImageLabel | ImageButton)
	...
	local objectIndex = table.find(self.Objects, object)
	...
	Log.Error(DebugStrings.ObjectIsNotAttached)
end
...
function Animator:ChangeAnimation(animationName: string)
	Log.TAssert(t.string(animationName))

	if self._AnimationPointer[animationName] == nil then
		Log.Error(DebugStrings.Animator.AnimationNotExist)
	end
