...
function Sound:_ApplyDirectionalData(scale: number, soundInstance: Sound)
	...
	SoundService:SetListener(Enum.ListenerType.CFrame, CFrame.new(CFrame.new().RightVector.X * -scale, 0, 0))
end
...
function Sound:PlayGlobal(origin: Vector2 | () -> Vector2)
	if not Settings.DirectionalSound.DirectionalAudioEnabled then
		Log.Warn(`Attemping to use :PlayGlobal(); DirectionalSound.DirectionalAudioEnabled is disabled however!`)
...
function Sound:Stop()
	-- self.Instance:Stop()
	for index, soundInstance: Sound in self.Instances do
